...
const UpdateOrder = () => {
    ...
    return (
        <>
            <MetaData title="Admin: Update Order | Edenzos" />

            {loading ? <Loading /> : (
                <>
                    {order && order.user && order.shippingInfo && (
                        <div className="flex flex-col gap-4">
                            <Link to="/admin/orders" className="ml-1 flex items-center gap-0 font-medium text-primary-pink uppercase"><ArrowBackIosIcon sx={{ fontSize: "18px" }} />Go Back</Link>
